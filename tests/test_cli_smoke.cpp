...
int main(int argc, char** argv) {
...

  {
    ...
    ok = ok && Check(Contains(telemetry_dump_bundle_r.output, "\"explain_schema_policy\":"),
                     "telemetry-dump-mounted --bundle reports explain schema policy");
    ...
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_confidence\":"),
                     "telemetry-dump-mounted --explain reports query_plan_confidence");
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_warnings\":"),
                     "telemetry-dump-mounted --explain reports query_plan_warnings");
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_profile\":"),
                     "telemetry-dump-mounted --explain reports query_plan_profile");
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_flags\":"),
                     "telemetry-dump-mounted --explain reports query_plan_flags");
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_rule_ids\":"),
                     "telemetry-dump-mounted --explain reports query_plan_rule_ids");
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_phase_ms\":"),
                     "telemetry-dump-mounted --explain reports query_plan_phase_ms");
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_phase_budget_ms\":"),
                     "telemetry-dump-mounted --explain reports query_plan_phase_budget_ms");
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_phase_status\":"),
    ...
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_operator_balance\":"),
                     "telemetry-dump-mounted --explain reports query_plan_operator_balance");
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_signature\":"),
    ...
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_negation_ratio\":"),
    ...
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"query_plan_selector_signature\":"),
                     "telemetry-dump-mounted --explain reports query_plan_selector_signature");
    ...
    ok = ok && Check(Contains(telemetry_dump_explain_r.output, "\"scan_match_ratio\":"),
    ...
    const auto telemetry_dump_where_icontains_cmd = Quote(exe_path.string()) +
                                                    " telemetry-dump-mounted Z: --where \"event_icontains==LOOP\" --bundle";
    const auto telemetry_dump_where_icontains_r = Run(telemetry_dump_where_icontains_cmd);
    ok = ok && Check(telemetry_dump_where_icontains_r.exit_code == 0,
                     "telemetry-dump-mounted event_icontains --where exits 0");
    ok = ok && Check(Contains(telemetry_dump_where_icontains_r.output,
    ...
    const auto telemetry_dump_where_ends_cmd = Quote(exe_path.string()) +
                                               " telemetry-dump-mounted Z: --where \"event_ends_with==loopback\" --bundle";
    const auto telemetry_dump_where_ends_r = Run(telemetry_dump_where_ends_cmd);
    ok = ok && Check(telemetry_dump_where_ends_r.exit_code == 0,
                     "telemetry-dump-mounted event_ends_with --where exits 0");
    ok = ok && Check(Contains(telemetry_dump_where_ends_r.output,
